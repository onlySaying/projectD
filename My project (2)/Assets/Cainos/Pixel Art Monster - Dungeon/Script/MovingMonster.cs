...
public class MovingMonster : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (attackable != 1)
        {
            ...
            if (target != null)
            {
                enemyVec = new Vector2(rigidbody.position.x + nextMove * 0.7f, rigidbody.position.y);
                Debug.DrawRay(enemyVec, Vector3.up, new Color(1, 0, 0));
                RaycastHit2D enemyFind = Physics2D.Raycast(enemyVec, Vector3.up, 1, LayerMask.GetMask("Player"));
                if (enemyFind.collider != null)
                {
                    attackP();
                }
            }
    ...
    {
        attackable = 1;
        anim.SetFloat("attackable",1);
        Invoke("attackComplete", 1.4f);
    }
    void attackComplete()
    {
        attackable = 0;
        speed = 3;
        anim.SetFloat("attackable",0);
        anim.SetFloat("speed", speed);
    }
